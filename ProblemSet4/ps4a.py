...
def getWordScore(word, n):
    ...
    points = 0
    for char in word:
        points += SCRABBLE_LETTER_VALUES.get(char, 0)
    if len(word) == n:
...
def updateHand(hand, word):
    ...
    #return {c: (hand.get(c,0) - getFrequencyDict(word).get(c,0)) for c in hand}
    return {c: hand.get(c,0) - word.count(c) for c in hand}
...
def isValidWord(word, hand, wordList):
    ...
    """
    return word in wordList and all([hand.get(l,0) >= word.count(l) for l in word])
...
def calculateHandlen(hand):
    ...
    returns: integer
    """
    # return sum([hand.get(c,0) for c in hand])
